...
vec4 direcLight()
{
    ...
    if(haveTextures == 1){
        ...
        resultColor = texDiffuse * (diffuse + ambientStrength) + vec4(texSpecular * specular, 0.0, 0.0, 0.0);
    }
    else{
        resultColor = vec4(1,1,1,1) * (diffuse + ambientStrength);
    }
    return resultColor * lightColor;
}
